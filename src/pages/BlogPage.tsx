...
// ─── Clé localStorage ─────────────────────────────────────────────────────────
const STORAGE_KEY = 'gua_blog_posts_v1';
...
function loadPosts(): Post[] {
  try {
    ...
    if (stored) {
      ...
      // Merge : garder les données initiales pour titre/body/images, mais conserver commentaires et réactions sauvegardés
      return initialPosts.map(init => {
        const saved = parsed.find(p => p.id === init.id);
        if (saved) return { ...init, reactions: saved.reactions, comments: saved.comments };
        return init;
      });
    }
  } catch { /* ignore */ }
      ...
      <div className="grid grid-rows-2 gap-2">
        {images.slice(1).map((img, i) => (
          <div key={i} className="cursor-pointer overflow-hidden rounded-xl" onClick={() => onOpen(i + 1)}>
            <img src={img} alt="" className="w-full h-[122px] object-cover hover:scale-105 transition-transform duration-300" />
          </div>
        ))}
      </div>
    </div>
...
function PostCard({
  ...
  onAddComment: (postId: number, author: string, text: string) => void;
}) {
  ...
  const visibleComments = showAllComments ? post.comments : post.comments.slice(-3);
  ...
  return (
    <article className="bg-white dark:bg-dark-surface rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
      ...
      <div className="px-5 pb-2 flex items-center justify-between text-xs text-gray-400">
        <span>{EMOJIS.slice(0, 3).join('')} {totalReactions} réactions</span>
      ...
      <div className="bg-white dark:bg-dark-surface border-b border-gray-100 dark:border-gray-800 py-12">
        <div className="max-w-2xl mx-auto px-4 text-center">
          ...
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-3">
            Ce qui se passe à <span className="text-primary">Green Up Academy</span>
          </h1>
      ...
      <div className="max-w-2xl mx-auto px-4 pb-16 space-y-6">
        ...
        {sorted.map((post, idx) => (
          <div key={post.id} className="relative">
            ...
            <PostCard post={post} onReact={handleReact} onAddComment={handleAddComment} />
          </div>
        ))}
        ...
        <div className="bg-gradient-to-br from-primary/10 to-emerald-50 dark:from-primary/5 dark:to-emerald-900/10 rounded-3xl p-6 text-center border border-primary/20">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Suivez-nous sur les réseaux pour ne rien manquer.</p>
